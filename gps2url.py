...
while True:
   ...
   if data[0:6] == "$GPRMC" and currenttm >= newread :
        ...
        lon = sdata[5] 		#longitude
        dirLon = sdata[6]      	#longitude direction E/W
        speed = sdata[7]       	#speed in knots
        trueCourse = sdata[8]   #true course
        dt = sdata[9][0:2] + "-" + sdata[9][2:4] + "-" + sdata[9][4:6] #date
        # print data        
        print("[Info] Date: %s time : %s, latitude : %s(%s), longitude : %s(%s), speed : %s,True Course : %s,"%   (dt,tm,lat,dirLat,lon,dirLon,speed,trueCourse))
        # Signature computation by SHA3-256
        tosign=lat+dirLat+lon+dirLon+socket.gethostname()+secretseed
        encoded_tosign = tosign.encode()
        bta = bytearray(encoded_tosign)
        signature=hashlib.sha3_256(bta).hexdigest()
        # replace variables
        buf=endpoint.replace("%LAT%",lat+dirLat)
        buf1=buf.replace("%LONG%",lon+dirLon)
        ...
        try:     
            with urllib.request.urlopen(endpoints) as response:
